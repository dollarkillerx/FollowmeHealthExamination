...
func TestEmail(t *testing.T) {
	d := gomail.NewDialer("smtp.mxhichina.com", 465, "postman@postman.cn", "postman")
	d.TLSConfig = &tls.Config{InsecureSkipVerify: true}

	m := gomail.NewMessage()
	m.SetHeader("From", "Postman"+"<postman@postman.cn>") //这种方式可以添加别名，即“XD Game”， 也可以直接用<code>m.SetHeader("From",mailConn["user"])</code> 读者可以自行实验下效果
	m.SetHeader("To", "postman@postman.com")              //发送给多个用户
	m.SetHeader("Subject", "Postman测试邮件")                 //设置邮件主题
	//m.SetBody("text/html", `hello world`)                      //设置邮件正文
	m.SetBody("text/html", p) //设置邮件正文
